...
// SCIENTIFIC MET VALUES FROM COMPENDIUM OF PHYSICAL ACTIVITIES
const WORKOUT_ACTIVITIES = [
  { name: "Running / Jogging (8 km/h)", met: 8.3 },
  { name: "Brisk Walking (5 km/h)", met: 4.3 },
  { name: "Gym Weightlifting / Resistance", met: 5.0 },
  { name: "Cycling / Biking (Moderate)", met: 7.5 },
  { name: "Jump Rope / Skipping", met: 10.0 },
  { name: "Calisthenics (Push-ups, Squats)", met: 6.0 }
];
...
export default function App() {
  ...
  // GPS LOCATION & STEP TRACKER STATES
  ...
  // WORKOUT COUNTDOWN TIMER TICK EFFECT
  useEffect(() => {
    if (isTimerRunning) {
  ...
  // SCIENTIFIC CALORIE BURN CALCULATION VIA MET FORMULA
  const calculateMetBurn = (metValue, durationMinutes) => {
    const userKg = appData?.weight || 60;
    // Formula: (MET * 3.5 * weight_kg / 200) * minutes
    return Math.round(((metValue * 3.5 * userKg) / 200) * durationMinutes);
  };
  ...
  const finishWorkoutSession = async () => {
    ...
    const burned = calculateMetBurn(actObj.met, elapsedMins);
  ...
  // DYNAMIC TARGET ADJUSTMENT INCLUDES ACTIVE BURNED CALORIES
  const baseActiveGoal = appData.dayMode === "workout" ? (appData.baseGoal || 2200) + 200 : (appData.baseGoal || 2200);
  const activeGoal = baseActiveGoal + (appData.todayBurnedCal || 0);
  ...
  // ESTIMATED STEPS FROM GPS DISTANCE (Average 1 km = 1350 steps)
  ...
  // LEGIT MET BURN DISPLAY DURING COUNTDOWN
  const liveWorkoutMetBurn = calculateMetBurn(activeWorkoutObj.met, Math.max(1, Math.round((initialMins * 60 - timerSeconds) / 60)));
      ...
      <div className="screen-container" style={{ padding: "16px", paddingBottom: "120px" }}>
        ...
        {activeTab === "home" && (
          <div className="screen active">
            ...
            {/* PHONE GPS LOCATION & STEP TRACKER CARD */}
            <div className="card" style={{ padding: "16px", borderRadius: "20px", background: "linear-gradient(135deg, #0284c7, #0369a1)", color: "white", marginBottom: "16px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "8px" }}>
                ...
                <div style={{ background: "rgba(255,255,255,0.1)", padding: "10px", borderRadius: "12px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: "18px", fontWeight: 900, marginTop: "2px" }}>🛣️ {totalGpsDistanceKm.toFixed(2)} km</div>
                </div>
              </div>
            ...
            {/* WORKOUT COUNTDOWN TIMER & LOG CARD WITH LEGIT MET FORMULA */}
            <div className="card" style={{ padding: "16px", borderRadius: "20px", background: "linear-gradient(135deg, #0f172a, #1e293b)", color: "white", marginBottom: "16px" }}>
              ...
              <div style={{ fontSize: "10px", color: "#94a3b8", fontWeight: 700, marginBottom: "10px" }}>
                Calorie burn calculated via ACSM MET guidelines based on body weight ({currentWeight} kg).
              </div>
              ...
              <div style={{ marginBottom: "10px" }}>
                ...
                >
                  {WORKOUT_ACTIVITIES.map(a => (
                    <option key={a.name} value={a.name}>{a.name} (MET {a.met})</option>
                  ))}
                </select>
              </div>
              ...
              <div style={{ background: "rgba(255,255,255,0.05)", padding: "14px", borderRadius: "14px", textAlign: "center", border: "1px solid rgba(255,255,255,0.1)", marginBottom: "12px" }}>
                ...
                <div style={{ fontSize: "11px", color: "#f59e0b", fontWeight: 800, marginTop: "4px" }}>
                  Est. Burn (MET): {liveWorkoutMetBurn} kcal
                </div>
            ...
            <div className="card">
              ...
              <div className="overview-grid">
                ...
                <div>
                  <p style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 600 }}>Calories Target (w/ Active Burn)</p>
...
print("App.jsx safe deployment ready!")
