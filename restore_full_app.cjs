...
export default function App() {
  ...
  const handleEmailAuth = async (e) => {
    e.preventDefault();
    if (!email || !password) return alert('Paki-sagutan ang Email at Password!');
    try {
      if (authMode === 'signup') {
  ...
  const setGoalPreset = (type) => {
    ...
    if (type === 'jogger') {
      baseGoal = tdee + 100; pGoal = Math.round(w * 1.8); cGoal = Math.round(w * 4.5);
    } else if (type === 'marathon') {
      baseGoal = tdee + 400; pGoal = Math.round(w * 1.8); cGoal = Math.round(w * 6.0);
    } else if (type === 'steps') {
      baseGoal = Math.max(1200, tdee - 250); pGoal = Math.round(w * 2.0); cGoal = Math.round(w * 3.2);
    } else if (type === 'bulk') {
      baseGoal = tdee + 250; pGoal = Math.round(w * 2.2); cGoal = Math.round(w * 4.0);
    } else if (type === 'cut') {
      baseGoal = Math.max(1200, tdee - 450); pGoal = Math.round(w * 2.4); cGoal = Math.round(w * 2.0);
    }
  ...
  if (loading) {
    return (
      <div className="mobile-frame" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p style={{ fontWeight: 800, color: 'var(--primary)' }}>Loading Cal Tracker...</p>
      </div>
    );
  }
      ...
      <div className="mobile-frame" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div className="card" style={{ maxWidth: '360px', width: '100%', padding: '24px' }}>
          <div style={{ textAlign: 'center', marginBottom: '20px' }}>
            <h2 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--primary)' }}>CAL TRACKER</h2>
          ...
          <div style={{ textAlign: 'center', marginTop: '16px' }}>
            <button onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} style={{ background: 'none', border: 'none', color: 'var(--primary)', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>
      ...
      <div className="screen-container">
        {activeTab === 'home' && (
          <div className="screen active">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <div>
                <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Fitness Dashboard</span>
        ...
        {activeTab === 'diary' && (
          <div className="screen active">
            ...
            <div className="card">
              <div style={{ fontSize: '14px', fontWeight: 800, marginBottom: '8px' }}>Logged Meals</div>
              <div>
        ...
        {activeTab === 'goals' && (
          <div className="screen active">
            ...
            <div className="goal-section-header">Select Fitness Strategy</div>
            ...
            <div className={"goal-card-option " + (appData.activeGoalType === 'steps' ? 'selected' : '')} onClick={() => setGoalPreset('steps')}>
              <div style={{ fontSize: '13px', fontWeight: 800 }}>Daily 10k Steps / Fat Loss</div>
              <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginTop: '2px' }}>Slight deficit for steady fat loss through daily walking.</div>
            </div>

            <div className={"goal-card-option " + (appData.activeGoalType === 'bulk' ? 'selected' : '')} onClick={() => setGoalPreset('bulk')}>
              <div style={{ fontSize: '13px', fontWeight: 800 }}>Lean Bulk (Muscle Gain)</div>
            ...
            <div className={"goal-card-option " + (appData.activeGoalType === 'cut' ? 'selected' : '')} onClick={() => setGoalPreset('cut')}>
              <div style={{ fontSize: '13px', fontWeight: 800 }}>Aggressive Cut</div>
...
console.log('App.jsx fully restored with all features!');
