...
// 3. Fix the syntax error near line 2005 (ISOLATED MODALS LAYER)
const modalMarker = '{/* ISOLATED MODALS LAYER */}';
const modalIndex = code.indexOf(modalMarker);

if (modalIndex !== -1) {
  // Kunin ang code bago ang Modals Layer
  let beforeModals = code.substring(0, modalIndex);
  const afterModals = code.substring(modalIndex);

  // Hanapin ang huling open block marker bago ang Modals Layer
  const lastTabStart = Math.max(
    beforeModals.lastIndexOf('{activeTab === "community"'),
    beforeModals.lastIndexOf('{activeTab === "pulse"'),
    beforeModals.lastIndexOf('{activeTab === "admin"')
  );

  if (lastTabStart !== -1) {
    // Rebuild clean Pulse & Admin section right before Modals Layer
    const cleanPulseAndAdmin = `{activeTab === "community" && (
    ...
    code = beforeModals.substring(0, lastTabStart) + cleanPulseAndAdmin + afterModals;
    fs.writeFileSync('src/App.jsx', code);
    console.log('✅ Syntax error fixed and Pulse Tab successfully restored!');
  }
} else {
  console.error('❌ Could not find ISOLATED MODALS LAYER marker');
}
