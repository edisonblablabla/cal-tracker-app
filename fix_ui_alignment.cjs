...
export default function App() {
  ...
  const createPost = async () => {
    if (!postText && !imagePreview) return alert('Please enter text or select an image.');
    try {
      const newPost = {
        userId: user.uid,
      ...
      fetchPosts();
      alert(postVisibility === 'private' ? 'Saved to Private Gallery.' : 'Published to Community Feed.');
    } catch (err) {
  ...
  return (
    <div className="mobile-frame" style={{ maxWidth: '480px', margin: '0 auto', background: '#f8fafc', minHeight: '100vh', position: 'relative' }}>
      <div className="screen-container" style={{ padding: '16px', paddingBottom: '80px' }}>
        {/* TAB 1: HOME */}
        {activeTab === 'home' && (
        ...
        {activeTab === 'diary' && (
          <div className="screen active">
            <h3 style={{ fontSize: '18px', fontWeight: 800, marginBottom: '14px' }}>Food & Nutrition Diary</h3>
            ...
            <div className="card">
              <div style={{ fontSize: '13px', fontWeight: 800, marginBottom: '10px' }}>Popular Meal Presets 🍛</div>
              ...
              <div className="preset-chip" onClick={() => addPreset('Saging na Saba (2 pcs - Pre-run fuel)', 180, 2, 45, 0)}>
                <div><span>Saging na Saba (Pre-Run Fuel)</span><br /><small style={{ color: 'var(--text-muted)', fontSize: '10px' }}>180 kcal • P:2g C:45g F:0g</small></div>
                <i className="fa-solid fa-plus" style={{ color: 'var(--primary)' }}></i>
              </div>
            </div>

            <div className="card">
              <div style={{ fontSize: '13px', fontWeight: 800, marginBottom: '10px' }}>Quick Meal Input</div>
              ...
              <button className="btn-block" onClick={addCustomMeal}>Log Meal</button>
            </div>
          ...
          <div className="screen active">
            ...
            <div className="card" style={{ padding: '16px', borderRadius: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', marginBottom: '16px' }}>
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '12px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'linear-gradient(135deg, #4f46e5, #818cf8)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '16px', overflow: 'hidden' }}>
                  {appData.avatarUrl ? (
                ...
                <div>
                  ...
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>Share an update or fitness milestone</div>
                </div>
              </div>
              ...
              <textarea 
                ...
                placeholder="Share your workout win or progress today..."
                value={postText}
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                <button 
                  ...
                  style={{ flex: 1, padding: '6px', borderRadius: '10px', fontSize: '11px', fontWeight: 800, border: '1px solid', borderColor: postVisibility === 'public' ? 'var(--primary)' : '#cbd5e1', background: postVisibility === 'public' ? '#e0e7ff' : '#ffffff', color: postVisibility === 'public' ? 'var(--primary)' : '#64748b', cursor: 'pointer' }}
                >
                  🌎 Public Feed
                </button>
                <button 
                  onClick={() => setPostVisibility('private')} 
                  style={{ flex: 1, padding: '6px', borderRadius: '10px', fontSize: '11px', fontWeight: 800, border: '1px solid', borderColor: postVisibility === 'private' ? 'var(--primary)' : '#cbd5e1', background: postVisibility === 'private' ? '#e0e7ff' : '#ffffff', color: postVisibility === 'private' ? 'var(--primary)' : '#64748b', cursor: 'pointer' }}
                >
                  🔒 Private Journal
                </button>
              </div>
        ...
        {/* TAB 6: PROFILE (PROPER ALIGNMENT FIX) */}
        {activeTab === 'profile' && (
          <div className="screen active">
            <div className="card" style={{ padding: 0, overflow: 'hidden', borderRadius: '24px', marginBottom: '16px', position: 'relative' }}>
              <div style={{ 
                ...
                <button 
                  ...
                  style={{ position: 'absolute', top: '12px', right: '12px', background: 'rgba(255,255,255,0.3)', backdropFilter: 'blur(4px)', color: 'white', border: 'none', width: '36px', height: '36px', borderRadius: '50%', cursor: 'pointer', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >
                  <i className="fa-solid fa-ellipsis-vertical"></i>
                </button>
              </div>

              {/* PERFECT FLEX ALIGNMENT FOR AVATAR */}
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 16px 16px 16px', marginTop: '-38px', textAlign: 'center' }}>
                <div style={{ width: '76px', height: '76px', borderRadius: '50%', background: '#ffffff', padding: '3px', boxShadow: '0 4px 15px rgba(0,0,0,0.1)', overflow: 'hidden', marginBottom: '8px' }}>
                  {appData.avatarUrl ? (
            ...
            <div className="card" style={{ padding: '16px', borderRadius: '20px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <span style={{ fontSize: '13px', fontWeight: 800, color: '#0f172a' }}>My Transformation Gallery</span>
              ...
              {myPosts.length === 0 ? (
                <p style={{ fontSize: '11px', color: 'var(--text-muted)', textAlign: 'center', padding: '20px 0' }}>No entries saved to your gallery yet.</p>
            ...
            {/* SETTINGS MODAL FIX WITH PROPER CENTERING & MAX HEIGHT */}
            {showSettingsModal && (
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
                <div className="card" style={{ width: '100%', maxWidth: '380px', padding: '20px', borderRadius: '24px', background: '#ffffff', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.2)' }}>
                  ...
                  <div style={{ marginBottom: '12px' }}>
                    <label style={{ fontSize: '11px', fontWeight: 700 }}>Profile Avatar</label>
                    <input type="file" accept="image/*" onChange={handleAvatarChange} className="form-input" style={{ fontSize: '10px' }} />
                  </div>

                  <div style={{ marginBottom: '12px' }}>
                    <label style={{ fontSize: '11px', fontWeight: 700 }}>Cover Banner Photo</label>
                    <input type="file" accept="image/*" onChange={handleCoverChange} className="form-input" style={{ fontSize: '10px' }} />
                  </div>
...
console.log('UI alignment bugs and modal overflow fixed successfully!');
