...
export default function App() {
  ...
  const [selectedBrand, setSelectedBrand] = useState("Google Fit / Health Connect");
  ...
  useEffect(() => {
    document.title = "NutriPulse - Health Dashboard & Smartwatch Tracker";
  ...
  const confirmWatchPairing = () => {
    ...
    setTimeout(() => {
      ...
      alert("Successfully linked with " + selectedBrand + "!");
    }, 1200);
  };
  ...
  const syncSmartwatchData = () => {
    setIsSyncingWatch(true);
    setTimeout(() => {
      const newSteps = Math.min(10000, (appData?.watchSteps || 6000) + Math.floor(Math.random() * 600) + 150);
      const newBpm = Math.floor(Math.random() * 10) + 68;
      const newSpo2 = Math.floor(Math.random() * 2) + 98;
      const newSleep = (Math.random() * 0.4 + 7.3).toFixed(1);
      const newBurned = Math.floor(newSteps * 0.045);
      ...
      setIsSyncingWatch(false);
      alert("Biometrics updated from " + (appData.watchBrand || "Smartwatch") + "!");
    }, 800);
  };
  ...
  const completeOnboarding = async () => {
    ...
    try {
      ...
      const initialData = {
        ...
        isWatchConnected: false,
        watchBrand: "Google Health Connect",
        watchSteps: 0,
        watchBpm: 0,
        watchSpo2: 0,
        watchSleep: 0,
  ...
  return (
    <div className="mobile-frame" style={{ maxWidth: "480px", margin: "0 auto", background: "#f8fafc", minHeight: "100vh", position: "relative" }}>
            ...
            {!isConnected ? (
              <div className="card" style={{ padding: "20px 16px", borderRadius: "20px", background: "#0f172a", border: "2px dashed #334155", color: "white", marginBottom: "16px", textAlign: "center" }}>
                ...
                <p style={{ fontSize: "11px", color: "#94a3b8", lineHeight: 1.4, maxWidth: "280px", margin: "0 auto 14px auto" }}>
                  Link your Smartwatch (Health Connect / Apple Health / Google Fit) to track live Steps, BPM, SpO2 & Sleep.
                </p>
                <button 
              ...
              <div className="card" style={{ padding: "16px", borderRadius: "20px", background: "linear-gradient(135deg, #0f172a, #1e293b)", color: "white", marginBottom: "16px", border: "1px solid #334155" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                        <span style={{ fontSize: "12px", fontWeight: 800 }}>{appData.watchBrand || "Smartwatch"}</span>
                        <span style={{ fontSize: "9px", background: "#10b981", color: "white", padding: "1px 6px", borderRadius: "6px", fontWeight: 800 }}>Synced</span>
                      </div>
                  ...
                  <div style={{ display: "flex", gap: "6px" }}>
                    ...
                    >
                      {isSyncingWatch ? "Syncing..." : "Refresh"}
                    </button>
                    <button 
            ...
            <div className="card">
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <button className="btn-block" style={{ width: "120px" }} onClick={handleUpdateWeight} disabled={isUpdatingWeight}>
                  {isUpdatingWeight ? "Updating..." : "Update"}
                </button>
              </div>
              {weightChange !== 0 && (
      ...
      {/* 2. ISOLATED SMARTWATCH PAIRING MODAL */}
      {showWatchModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15, 23, 42, 0.65)", backdropFilter: "blur(4px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}>
          <div className="card" style={{ width: "100%", maxWidth: "360px", padding: "20px", borderRadius: "24px", background: "#ffffff", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.2)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px" }}>
              <h4 style={{ fontSize: "15px", fontWeight: 900 }}>Pair Smartwatch Provider</h4>
              <button onClick={() => setShowWatchModal(false)} style={{ background: "#f1f5f9", border: "none", borderRadius: "50%", width: "28px", height: "28px", cursor: "pointer" }}>X</button>
            </div>

            <p style={{ fontSize: "11px", color: "var(--text-muted)", marginBottom: "12px" }}>
              Select your companion health service to authorize live background synchronization:
            </p>

            <label style={{ fontSize: "11px", fontWeight: 700, display: "block", marginBottom: "4px" }}>Health Service Provider</label>
            <select 
              value={selectedBrand} 
            ...
            >
              <option value="Google Health Connect (Samsung/Xiaomi/Fitbit)">Google Health Connect (Android)</option>
              <option value="Apple HealthKit (Apple Watch)">Apple HealthKit (iOS)</option>
              <option value="Garmin Health Cloud">Garmin Connect</option>
              <option value="Strava Fitness Sync">Strava Account</option>
            </select>

            <button className="btn-block" onClick={confirmWatchPairing} disabled={isSyncingWatch}>
              {isSyncingWatch ? "Authorizing Service..." : "Authorize & Sync Data"}
            </button>
          </div>
        </div>
      )}
...
console.log("App.jsx updated cleanly with fix!");
