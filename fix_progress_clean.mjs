...
// Split code into before and after the progress tab to avoid regex corruption
const progressMarker = '{activeTab === "progress" && (';
const goalsMarker = '{activeTab === "goals" && (';

const startIndex = code.indexOf(progressMarker);
const endIndex = code.indexOf(goalsMarker);

if (startIndex !== -1 && endIndex !== -1) {
  const beforeProgress = code.substring(0, startIndex);
  const afterProgress = code.substring(endIndex);

  const cleanProgressContent = `{activeTab === "progress" && (
  ...
  code = beforeProgress + cleanProgressContent + afterProgress;
  fs.writeFileSync('src/App.jsx', code);
  console.log('✅ Clean Progress Tab replaced without syntax errors!');
} else {
  console.error('❌ Could not find Progress or Goals tab markers in src/App.jsx');
}
