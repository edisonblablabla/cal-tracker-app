...
export default function App() {
  ...
  const createPost = async () => {
    if (!postText && !postImageUrl) return alert('Please enter text or photo URL');
    try {
      const newPost = {
        userId: user.uid,
      ...
      setPostImageUrl('');
      fetchPosts();
      alert('Transformation Post Published!');
    } catch (err) {
  ...
  return (
    <div className="mobile-frame">
      <div className="screen-container">
        ...
        {activeTab === 'community' && (
          <div className="screen active">
            <h3 style={{ fontSize: '18px', fontWeight: 800, textAlign: 'center', marginBottom: '14px' }}>Community Inspiration</h3>

            <div className="card" style={{ background: '#f8fafc' }}>
              <div style={{ fontSize: '13px', fontWeight: 800, marginBottom: '8px' }}>Share Your Win / Transformation</div>
              <textarea 
                className="form-input" 
                style={{ height: '60px', marginBottom: '8px', resize: 'none' }} 
                placeholder="Write your daily progress or motivation..."
                value={postText}
              ...
              <input 
                ...
                placeholder="Image URL (Optional, e.g. Before/After photo)" 
                value={postImageUrl}
                onChange={e => setPostImageUrl(e.target.value)}
              />
              <button className="btn-block" onClick={createPost}>Post to Community</button>
            </div>

            <div>
              {posts.length === 0 ? (
                <p style={{ textAlign: 'center', fontSize: '12px', color: 'var(--text-muted)' }}>No posts yet. Be the first to share your win!</p>
              ) : (
                posts.map(p => (
                  <div key={p.id} className="card" style={{ marginBottom: '12px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--primary)' }}>{p.userName}</span>
                      <small style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{new Date(p.createdAt).toLocaleDateString()}</small>
                    </div>
                    {p.text && <p style={{ fontSize: '12px', color: '#1e293b', marginBottom: '8px' }}>{p.text}</p>}
                    {p.imageUrl && (
                      <img src={p.imageUrl} alt="Transformation Post" style={{ width: '100%', borderRadius: '12px', marginBottom: '8px', maxHeight: '200px', objectFit: 'cover' }} />
                    )}
                    <button 
                      onClick={() => handleLike(p.id, p.likes || 0, p.likedBy || [])}
                      style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#ef4444', borderRadius: '20px', padding: '4px 12px', fontSize: '11px', fontWeight: 700, cursor: 'pointer' }}
                    >
                      <i className="fa-solid fa-heart" style={{ marginRight: '4px' }}></i> {p.likes || 0} Likes
                    </button>
                  </div>
                ))
              )}
        ...
        {activeTab === 'profile' && (
          <div className="screen active">
            ...
            <div className="card">
              <div style={{ fontSize: '14px', fontWeight: 800, marginBottom: '12px' }}>My Posts & Transformation Gallery</div>
              {myPosts.length === 0 ? (
                <p style={{ fontSize: '11px', color: 'var(--text-muted)', textAlign: 'center' }}>You haven't posted anything to the community yet.</p>
              ) : (
                myPosts.map(p => (
                  <div key={p.id} style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '8px' }}>
                    <p style={{ fontSize: '11px', fontWeight: 700 }}>{p.text}</p>
                    <small style={{ fontSize: '9px', color: 'var(--text-muted)' }}>❤️ {p.likes || 0} Likes</small>
                  </div>
                ))
              )}
            </div>
...
console.log('Community Feed and consolidated Diary syntax completely fixed!');
