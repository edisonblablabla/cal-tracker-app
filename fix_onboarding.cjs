...
export default function App() {
  ...
  // Onboarding States
  const [onboardStep, setOnboardStep] = useState(0); // 0: Check, 1: Disclaimer, 2: Setup Profile
      ...
      if (currentUser) {
        ...
        if (docSnap.exists()) {
          ...
          if (!data.onboardingCompleted) {
            setOnboardStep(1); // Go to Disclaimer
          } else {
            setOnboardStep(0); // Normal App Access
          }
        } else {
          // Brand New User
  ...
  const completeOnboarding = async () => {
    ...
    setProfWeight(w);
    setProfActivity(act);
    setOnboardStep(0); // Launch Main App
  };
  ...
  // STEP 1: DISCLAIMER MODAL FOR NEW USER
  if (onboardStep === 1) {
    return (
      <div className="mobile-frame" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
  ...
  // STEP 2: PROFILE & GOALS SETUP FOR NEW USER
  if (onboardStep === 2) {
    return (
...
console.log('App.jsx with onboarding steps updated!');
